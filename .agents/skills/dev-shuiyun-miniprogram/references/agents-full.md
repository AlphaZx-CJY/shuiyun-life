...
| 活动详情   | `pages/schedule-detail/schedule-detail` | `schedules` 集合                       | 单条活动详情                              |
| 班车信息   | `pages/shuttle/shuttle`                 | `shuttle_config` + `shuttle_times`     | 时刻表、站点、动态状态计算                |
| 社区心声   | `pages/voice/voice`                     | `voices` 集合                          | 用户心声列表                              |
| 发布心声   | `pages/voice-publish/voice-publish`     | 写入 `voices` 集合                     | 提交社区心声                              |
