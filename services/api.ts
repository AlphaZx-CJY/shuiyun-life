...
export async function getRecentSchedules(limit = 5): Promise<Pick<ScheduleItem, 'id' | 'title' | 'time' | 'location' | 'status' | 'date'>[]> {
  ...
  const data = await safeQuery<ScheduleItem>('schedules', { enabled: true });
  const now = new Date();
  return data
    ...
    .slice(0, limit)
    .map((s) => {
      const itemDateTime = new Date(`${s.startDate}T${s.time}`);
...
/** 规范化活动数据：将 startDate/endDate 组装为 date 展示字段，兼容旧数据 */
function normalizeSchedule(item: ScheduleItem): ScheduleItem {
  const startDate = item.startDate || item.date;
  const endDate = item.endDate || startDate;
  const date = startDate === endDate ? startDate : `${startDate} ~ ${endDate}`;
  return { ...item, startDate, endDate, date };
}
...
export async function getScheduleDetail(_id: number | string): Promise<ScheduleItem | null> {
  try {
    ...
    return resolveCloudUrls(normalizeSchedule(list[0]!));
  } catch (e) {
